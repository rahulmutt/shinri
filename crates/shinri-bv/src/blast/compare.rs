...
pub fn ult(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    ult_core(b, x, y)
}
pub fn ugt(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    ult(b, y, x)
}
pub fn ule(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    let g = ugt(b, x, y);
    b.not1(g)
}
pub fn uge(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    let l = ult(b, x, y);
    b.not1(l)
}
...
pub fn sgt(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    slt(b, y, x)
}
pub fn sle(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    let g = sgt(b, x, y);
    b.not1(g)
}
pub fn sge(b: &mut Blaster, x: &[BitLit], y: &[BitLit]) -> BitLit {
    let l = slt(b, x, y);
    b.not1(l)
}
    ...
    fn unsigned_and_signed_compares() {
        ...
        for &(x, y) in &signed_cases {
            ...
            // slt: x < y (signed)
            {
                ...
                assert_eq!(
                    got, expected,
                    "slt({:02x},{:02x}) = slt({},{}) expected {} got {}",
                    x, y, xi, yi, expected, got
                );
            }
            // sle: x <= y (signed)
            {
