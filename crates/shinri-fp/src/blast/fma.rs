...
pub fn fp_fma(
    ...
    let (hi_sign, hi_exp, hi_sig) = select3(
        b,
        p_ge_z,
        (prod_sign, &prod_exp, &prod_n),
        (z_sign, &z_exp, &z_sig),
        ew,
        pw,
    );
    let (lo_sign, lo_exp, lo_sig) = select3(
        b,
        p_ge_z,
        (z_sign, &z_exp, &z_sig),
        (prod_sign, &prod_exp, &prod_n),
    ...
    let mut lo_ext: Vec<BitLit> = vec![zb; 3];
    lo_ext.extend_from_slice(&lo_sig); // mw
    ...
    for i in 0..mw {
        let hb = if i + 1 < mw { mant[i + 1] } else { add_carry };
        mant_a.push(hb);
    }
    ...
    let mut s = b.zero();
    for bit in mant_n.iter().take(mw - sbu - 2) {
        s = b.or2(s, *bit);
    }
    let ext = ExtFp {
        sign: res_sign,
...
fn select3(
    b: &mut Blaster,
    sel: BitLit,
    a: (BitLit, &[BitLit], &[BitLit]),
    c: (BitLit, &[BitLit], &[BitLit]),
    ew: usize,
    pw: usize,
) -> (BitLit, Vec<BitLit>, Vec<BitLit>) {
...
fn special_case(
    ...
    let any_nan = {
        let t = b.or2(ox.is_nan, oy.is_nan);
        b.or2(t, oz.is_nan)
    };
    ...
    let want_nan = {
        let t = b.or2(any_nan, invalid);
        b.or2(t, inf_sign_clash)
    };
