...
use crate::blast::operand::{canon_nan_bits, signed_zero_bits, to_operand};
use crate::lzc::lzc;
...
pub fn fp_rem(b: &mut Blaster, x: &[BitLit], y: &[BitLit], eb: u32, sb: u32) -> Vec<BitLit> {
    ...
    for &bit in &hx_lo {
        hx_nz = b.or2(hx_nz, bit);
    }
    let r_zero = b.not1(hx_nz); // exact division (ed>=0)

    // --- ed < 0 path: residue = |x|, already normalized (rsig=Mx, rexp=ex) -----
    let rsig: Vec<BitLit> = (0..sbu)
        .map(|j| b.mux2(ed_neg, mx[j], rsig_ge0[j]))
        .collect();
    let rexp: Vec<BitLit> = (0..ew)
        .map(|j| b.mux2(ed_neg, ex[j], rexp_ge0[j]))
        .collect();
    ...
    let mag_sig: Vec<BitLit> = (0..sbu)
        .map(|j| b.mux2(inc, diff_sig[j], rsig[j]))
        .collect();
    ...
    let ext = ExtFp {
        ...
        exp: mag_exp,
        sig: mag_sig,
        grs: (zero_bit, zero_bit, zero_bit),
    };
    let normal_path = round(
        b,
        ext,
        eb,
        sb,
        &crate::rm::literal(b, shinri_core::RoundingMode::Rne),
    );
    ...
    for i in 0..w {
        out[i] = b.mux2(r_zero, zsign[i], out[i]);
    }
    // rem(x, inf) = x  (x finite here; specials below override for inf/nan/zero x).
    for i in 0..w {
        out[i] = b.mux2(oy.is_inf, x[i], out[i]);
    }
    // rem(±0, y) = ±0 (sign of x).
    for i in 0..w {
        out[i] = b.mux2(ox.is_zero, zsign[i], out[i]);
    }
    // rem(_, 0) -> NaN ; rem(inf, _) -> NaN ; any NaN -> NaN.
    let nan = canon_nan_bits(b, eb, sb);
    for i in 0..w {
        out[i] = b.mux2(oy.is_zero, nan[i], out[i]);
    }
    for i in 0..w {
        out[i] = b.mux2(ox.is_inf, nan[i], out[i]);
    }
    for i in 0..w {
        out[i] = b.mux2(ox.is_nan, nan[i], out[i]);
    }
    for i in 0..w {
        out[i] = b.mux2(oy.is_nan, nan[i], out[i]);
    }
    out
}
...
mod tests {
    ...
    fn rem_float32_specials_and_random() {
        ...
        for iter in 0..600 {
            let (a, bb) = if iter < cases.len() {
                cases[iter]
            } else {
                (rand() & 0xFFFF_FFFF, rand() & 0xFFFF_FFFF)
            };
            let want = ref_rem(eb, sb, &Integer::from(a), &Integer::from(bb));
            let mut b = Blaster::new();
            let xw = const_bits(&b, eb, sb, a);
