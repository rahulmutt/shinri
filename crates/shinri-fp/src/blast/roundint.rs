...
use crate::blast::operand::{
    canon_nan_bits, inf_pattern_bits, signed_one_bits, signed_zero_bits, to_operand,
};
use crate::rm::RmSel;
use crate::round::{exp_w, rounding_increment, shift_right_sticky};
use shinri_bv::blast::arith::{adder, bvadd};
use shinri_bv::blast::shift::bvshl;
use shinri_bv::{BitLit, Blaster};

pub fn fp_round_to_integral(
    b: &mut Blaster,
    x: &[BitLit],
    rm: &RmSel,
    eb: u32,
    ...
    let f_sat: Vec<BitLit> = (0..ew)
        .map(|i| b.mux2(f_neg, zero_ew[i], f_full[i]))
        .collect();
    ...
    let f_shl: Vec<BitLit> = (0..(sbu + 1))
        .map(|i| if i < ew { f_sat[i] } else { b.zero() })
        .collect();
    let shifted_back = bvshl(b, &ir_ext, &f_shl); // sbu+1 bits
    let overflow = shifted_back[sbu];
    let norm_sig: Vec<BitLit> = (0..sbu)
        .map(|i| {
    ...
    let exp_out: Vec<BitLit> = (0..ew)
        .map(|i| b.mux2(overflow, exp_p1[i], ox.exp[i]))
        .collect();
    ...
    let lt1: Vec<BitLit> = (0..w)
        .map(|i| b.mux2(inc, one_bits[i], zero_bits[i]))
        .collect();
    for i in 0..w {
        out[i] = b.mux2(is_lt1, lt1[i], out[i]);
    }
...
mod tests {
    ...
    fn roundint_float32_specials_and_random() {
        ...
        let cases: &[u64] = &[
            ...
            0x4B7F_FFFF,
            0x4C00_0000, // large integral magnitudes
        ];
        ...
        for iter in 0..2000 {
            let bits = if iter < cases.len() as u64 {
                cases[iter as usize]
            } else {
                rand() & 0xFFFF_FFFF
            };
