...
pub fn is_nan(_b: &mut Blaster, u: &Unpacked) -> BitLit {
    u.is_nan
}
pub fn is_inf(_b: &mut Blaster, u: &Unpacked) -> BitLit {
    u.is_inf
}
pub fn is_zero(_b: &mut Blaster, u: &Unpacked) -> BitLit {
    u.is_zero
}
...
pub fn is_normal(b: &mut Blaster, u: &Unpacked) -> BitLit {
    ...
    for &e in &u.exp {
        all_ones = b.and2(all_ones, e);
    }
    let mut all_zero = b.one();
    for &e in &u.exp {
        let ne = b.not1(e);
        all_zero = b.and2(all_zero, ne);
    }
...
pub fn is_subnormal(b: &mut Blaster, u: &Unpacked) -> BitLit {
    ...
    for &s in &u.sig {
        let ns = b.not1(s);
        sig_all_zero = b.and2(sig_all_zero, ns);
    }
...
mod tests {
    use super::*;
    use crate::reference::{
        decode, ref_is_inf, ref_is_nan, ref_is_negative, ref_is_normal, ref_is_positive,
        ref_is_subnormal, ref_is_zero,
    };
