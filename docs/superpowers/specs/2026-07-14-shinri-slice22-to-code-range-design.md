...
- **The flip.** `targeted_code_conv_fences_unknown`
  (`qfs_differential.rs:2641`) pins `(>= (str.to_code s) 48)` at `Unknown`
  (`:2681`). That case flips to decided — the house canary idiom. The
  nested-arith case (`:2673`) stays pinned `Unknown`.
- The digit idiom `48 ≤ to_code(s) ≤ 57`: sat with a get-value witness; unsat
  when conjoined with `s = "x"`.
- One pin on **each** downstream route — a narrow fused range (slice-20
  enumeration) and a wide one (slice-21 engine) — so a future change that
  silently reroutes them trips a test.
